...
import { mergeItems, parseItem } from "./scraper.js";

test("mergeItems - basic merging with preposition", () => {
  const input = ["Fullkorn pasta Bolognese med", "parmesan 1,3,4"];
...
test("mergeItems - multiple merges", () => {
  const input = ["Kylling og", "ris", "med salat"];
...
test("mergeItems - Norwegian characters", () => {
  const input = ["Laks på", "eng av grønnsaker"];
